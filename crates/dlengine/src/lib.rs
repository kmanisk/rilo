...
impl DlmanCore {
    ...
    pub async fn add_download_with_options(
        ...
        let unique_filename = Self::get_unique_filename(&destination, &init_resolved.resolved_filename, self.download_manager.db()).await;
        ...
        eprintln!("[ADD-DOWNLOAD] id={} url={}", download.id, url);
        ...
        if auto_start {
            let core = self.clone();
            let id = download.id;
            eprintln!("[DOWNLOAD-TASK-START] id={}", id);
            tokio::spawn(async move {
                if let Err(e) = core.resume_download(id).await {
                    eprintln!("[ERROR] id={} resume_download failed: {}", id, e);
    ...
    /// Resume a download with optional requested segment count
    pub async fn resume_download_with_options(&self, id: Uuid, requested_segments: Option<u32>) -> Result<(), DlmanError> {
        eprintln!("[PROBE-START] id={} requested_segments={:?}", id, requested_segments);
        ...
        } else if let Some(req_seg) = requested_segments {
            req_seg.clamp(1, 32)
        } else {
    ...
    pub async fn resume_download(&self, id: Uuid) -> Result<(), DlmanError> {
        self.resume_download_with_options(id, None).await
    }
