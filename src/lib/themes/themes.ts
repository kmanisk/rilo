...
const light = (background:string, surface:string, elevated:string, text:string, secondary:string, muted:string, accent:string): Record<string,string> => ({...dark(background,surface,elevated,text,secondary,muted,accent,"#1a7f37","#9a6700","#cf222e"),surfaceHover:"#eaeea0",surfaceActive:"#d0d7de",border:"#d0d7de",borderStrong:"#afb8c1",accentForeground:"#ffffff",selection:"rgba(9,105,218,.14)"});
...
export const riloThemes: RiloTheme[] = [
  ...
  },
  {
    ...
    category: "dark",
    colors: dark("#1f2335","#24283b","#2f354f","#c0caf5","#a9b1d6","#737aa2","#7aa2f7","#9ece6a","#e0af68","#f7768e"),
    lightColors: light("#e1e2e7","#f3f4f7","#ffffff","#373e4d","#565f89","#7a88cf","#7aa2f7"),
  },
  {
    id: "gruvbox-dark",
    name: "Gruvbox Dark",
    category: "dark",
    colors: dark("#282828","#3c3836","#504945","#ebdbb2","#d5c4a1","#a89984","#83a598","#b8bb26","#fabd2f","#fb4934"),
  ...
  },
  {
    ...
    category: "dark",
    colors: dark("#24273a","#1e2030","#363a4f","#cad3f5","#b8c0e0","#8087a2","#8aadf4","#a6da95","#eed49f","#ed8796"),
    lightColors: light("#eff1f5","#ffffff","#e6e9ef","#4c4f69","#5c5f77","#8c8fa1","#1e66f5"),
  },
  {
    id: "catppuccin-latte",
  ...
  },
  {
    ...
    name: "Ayu Mirage",
    category: "dark",
    colors: dark("#1f2430","#242936","#303747","#cccac2","#b8c0cc","#707a8c","#ffcc66","#bae67e","#ffd580","#ff6666"),
    lightColors: light("#f8f9fa","#ffffff","#f3f4f5","#5c6166","#8a9199","#abb0b6","#ff9900"),
  },
  {
    id: "nord",
...
export const getDarkThemes = () => riloThemes.filter((t) => (t.category || "dark") === "dark");
export const getLightThemes = () => riloThemes.filter((t) => t.category === "light");

export const getRiloTheme = (id: string, mode: "dark" | "light" = "dark") => {
  let targetId = id;
  if (id === "dark") targetId = "rilo-default";
  if (id === "light") targetId = "github-light";

  const theme = riloThemes.find((t) => t.id === targetId) || riloThemes[0];

  if (mode === "light") {
    if (theme.lightColors) {
      return { id: theme.id, name: theme.name, colors: theme.lightColors, isLight: true };
    }
    if (theme.category === "light") {
      return { id: theme.id, name: theme.name, colors: theme.colors, isLight: true };
    }
    const ghLight = riloThemes.find((t) => t.id === "github-light");
    if (ghLight) return { id: theme.id, name: theme.name, colors: ghLight.colors, isLight: true };
  }

  return { id: theme.id, name: theme.name, colors: theme.colors, isLight: mode === "light" };
};
